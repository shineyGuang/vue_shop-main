<template>
  <div class="RobotEdit">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>更新机器人</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card class="robot-card">
      <div slot="header" class="clearfix">
        <span id="robot-name">{{ this.robotName }}</span>
        <el-button style="float: right; padding: 3px 0" type="success" :disabled="clickable" @click="submitEditRobot">
          确认更新
        </el-button>
      </div>
      <el-form ref="editRobotFormRef" status-icon :model="form" :rules="editRobotFormRules" class="registerForm">
        <el-row :gutter="20">
          <el-col :span="20">
            <!--机器人名称填写框-->
            ...
            <!--机器人图片展示-->
            <el-form-item prop="image" label="机器人APP图片修改">
              <el-col>
            ...
            <el-row>
              <el-col :span="10" class="upload-box">
                ...
                <!--                <my-video-player class="my-video-player" :video-url="this.form.video"></my-video-player>-->
                <el-upload
                  class="upload-demo"
                  ref="editVideoUpload"
...
export default {
  ...
  created() {
    this.robotName = window.sessionStorage.getItem('CurRobotEditName')
  ...
  methods: {
    clearImg() {
      this.$refs.editImgUpload.clearFiles()
    },
    clearFiles() {
      this.$refs.editVideoUpload.clearFiles()
    },
    submitEditRobot() {
      this.$refs.editRobotFormRef.validate(async valid => {
        console.log(valid)
        if (!valid) {
          ...
          const { data: res } = await this.$http.put(`/robot/editOneRobot/${this.robotName}`, formData)
          if (res.statusCode === 20031) {
            this.$message.success('机器人更新成功')
    ...
    async getRobot() {
      ...
      // console.log(res)
      this.form.robot_name = res.data[0].robot_name
      this.form.category_id = res.data[0].category__category_id
      this.form.download_counter = res.data[0].download_counter
      this.form.rate = res.data[0].rate
      this.form.cur_version = res.data[0].details_robot__cur_version
      this.form.image = res.data[0].image
      this.form.video = res.data[0].details_robot__video
      this.form.publisher_id = res.data[0].publisher_id
      this.publisherName = res.data[0].publisher__username
      this.form.content = res.data[0].details_robot__content
      this.form.c_html = res.data[0].details_robot__c_html
      this.form.manual = res.data[0].details_robot__manual
      this.form.m_html = res.data[0].details_robot__m_html
      this.form.keywords = res.data[0].details_robot__keywords
